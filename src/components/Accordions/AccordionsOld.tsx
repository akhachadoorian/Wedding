...
export type AccordionOldProps = {
    className?: string;
    question: string;
    answer: string;
};

export function AccordionOld({ className, question, answer }: AccordionOldProps) {
...
export type AccordionsProps = {
    className?: string;
    accordions: Array<AccordionOldProps>;
};
        ...
        <div className={`accordions ${className ?? ""}`}>
            <div className="accordions-inner">
                {accordions.map((a, idx) => (
                    <AccordionOld key={idx} question={a.question} answer={a.answer} />
                ))}
            </div>
        </div>
    );
